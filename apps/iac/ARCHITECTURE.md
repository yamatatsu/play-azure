...
| **NAT** | NAT Gateway | 固定送信元IP (staging/prod) |
| **コンピュート** | Azure Container Apps (Consumption) | アプリケーションホスティング |
| **レジストリ** | Azure Container Registry (Standard) | Dockerイメージ保管 |
...
- **キュー処理ワーカー**: 非同期タスクを処理するバックグラウンドワーカー（将来実装予定）
...
デプロイ: staging/prod環境のみ（開発環境はコスト削減のため無効）
```
...
構成:
- SKU: Standard
- Admin User: 有効（開発時の利便性）
- パブリックアクセス: 有効
- ゾーン冗長: 無効（Premium SKUで利用可能）

認証:
- Container Apps: Managed Identity経由でAcrPullロール
- 開発者: Azure CLI経由 (az acr login)
```

**命名規則:**
- ACR名はグローバルに一意かつハイフン不可
- 例: `yamatatsu-lab-v1-dev` → `yamatatsuv1devacr`

### 4.4 PostgreSQL Flexible Server

```
構成:
- SKU: General Purpose (本番) / Burstable (開発)
- バージョン: PostgreSQL 15
- HA: Zone-Redundant (prod環境のみ)
...
- 自動フェイルオーバー（Zone-Redundant HA、prod環境）
...
### 4.5 Container Apps - Web Server

```
構成:
- Ingress: External (HTTPS公開)
- Port: 3000
- プロトコル: HTTP（内部）、HTTPS（外部からのアクセス）
- TLS: Azureが自動管理
- スケール:
  - Min Replicas: 0 (トラフィックなし時は停止)
  - Max Replicas: 10
  - Scale Rule: HTTP Concurrent Requests (10リクエスト/コンテナ)
- リソース:
  - CPU: 0.5 vCPU
  - Memory: 1Gi
- ヘルスチェック:
...
### 4.6 Container Apps - Queue Worker
...
### 4.7 Container Apps Environment
...
- 監視: Log Analytics + Application Insights統合
```
...
- IP制限（ipSecurityRestrictions）可能
- Easy Auth（必要に応じて）
...
#### 転送中の暗号化
```
✓ Container Apps: HTTPS/TLS 1.2+必須（外部アクセス）
✓ PostgreSQL: SSL/TLS接続必須 (sslmode=require)
✓ VNet内通信: プライベートネットワーク
✓ ACR: HTTPS経由でイメージpull
```
...
```
影響: データベース接続不可
対応: Zone-Redundant HAによる自動フェイルオーバー（prod環境）
ダウンタイム: 60-120秒
```
...
PostgreSQL: スタンバイへフェイルオーバー（prod環境）
ダウンタイム: 数分以内
```
...
NAT Gateway:
- 基本料金 + データ処理: ¥5,000-10,000/月

PostgreSQL Flexible Server (General Purpose):
- 基本料金 + ストレージ: ¥30,000-50,000/月
- Zone-Redundant HA: +50%
...
### 9.1 Application Gateway + WAF 追加
